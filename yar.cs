...
public class Server 
{
  // used to pass state information to delegate
	...
	static void Main() 
	{
		...
		IAsyncResult asyncAccept = listenSocket.BeginAccept(new AsyncCallback(Server.acceptCallback), listenSocket );
	...
	public static void acceptCallback(IAsyncResult asyncAccept) 
	{
		...
		IAsyncResult asyncReceive = serverSocket.BeginReceive(stateObject.sBuffer, 0, stateObject.sBuffer.Length, SocketFlags.None, new AsyncCallback(receiveCallback), stateObject);
	...
	public static void receiveCallback(IAsyncResult asyncReceive) {
		...
		Console.WriteLine(".{0} bytes received: {1}", bytesReceived.ToString(), Encoding.ASCII.GetString(stateObject.sBuffer) );

		byte[] sendBuffer = Encoding.ASCII.GetBytes("Goodbye");
		IAsyncResult asyncSend = stateObject.sSocket.BeginSend(sendBuffer, 0, sendBuffer.Length, SocketFlags.None, new AsyncCallback(sendCallback), stateObject.sSocket);
	...
	public static void sendCallback(IAsyncResult asyncSend) 
	{
		...
		Console.WriteLine(".{0} bytes sent.{1}{1}Shutting down.", bytesSent.ToString(), Environment.NewLine);
